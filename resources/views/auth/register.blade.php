...
@section('content')
     ...
     <form method="POST" action="{{ route('register') }}">
        ...
        <!-- Role -->
        <div class="mt-4">
            <x-input-label class="sr-only" for="role" :value="__('Role')" />
            <select id="role" name="role" class="block mt-1 w-full select select-bordered hidden" required>
                <option value="customer" {{ old('role') === 'customer' ? 'selected' : '' }}>Customer</option>
                {{-- <option value="seller" {{ old('role') === 'seller' ? 'selected' : '' }}>Seller</option>
                <option value="admin" {{ old('role') === 'admin' ? 'selected' : '' }}>Admin</option> --}}
                <!-- 'admin' omitted for public registration; handle via seeder or admin panel -->
            </select>
